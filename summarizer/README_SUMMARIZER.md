...
즉, **Summarizer 요청 시 transcript는 Spring이 전달**하며,  
AI 서버는 DB를 조회하지 않는다.
...
4. Spring이 이 결과를 `summary` 테이블과 `consultations.summary` 캐시에 반영한다.

요약은 **환자용** 기준으로 설계하며, `doctor_summary` 는 생성하지 않는다.
...
> 🔑 중요한 점  
> - STT 결과(STT segment, transcript)는 **Spring이 관리**한다.  
...
LLM 프롬프트/호출에 사용하는 transcript는 위 구조에서 `transcript` 필드이며,  
언어 정보는 `language` 또는 meta.language를 참고한다.
...
Spring은 Summarizer 응답을 받아 다음을 수행한다:
...
1. `summary` 테이블에 INSERT
   ...
   - `tags` ← `data.tags`
2. (선택) `consultations.summary` 컬럼에 `data.content` 저장
3. 외부 API (`GET /summaries/{consultationId}`) 응답으로  
   `content` / `tags` / `createdAt` 등을 내려준다.

STT 결과(`transcript`)가 없는 경우:

- Spring에서 `recordings.status != 'TRANSCRIBED'` 또는 `transcript IS NULL` 이면
- `/internal/summarize` 를 호출하지 않고,  
  클라이언트에게 적절한 에러(예: `SUMMARY_NOT_READY` 등)를 반환하는 것이 기본 흐름이다.

---

## 7. Error Response 정의

### 7.1 잘못된 요청 (400 Bad Request)
...
### 7.2 내부 오류 (500 Internal Server Error)
